...
public class DeleteTaskByNodeCommand implements Command<Integer> {
	...
	public Integer execute(Context context) {
		...
		List<Task> list=session.createQuery(hql, Task.class)
				.setParameter("nodeName", nodeName)
				.setParameter("processInstanceId", processInstanceId)
				.setParameter("rootProcessInstanceId", processInstanceId)
				.list();
		...
		for(Task task:list){
			if(task.getType().equals(TaskType.Participative)){
				Query<?> query1 = session.createQuery("delete "+TaskParticipator.class.getName()+" where taskId=:taskId");
				query1.setParameter("taskId", task.getId());
				query1.executeUpdate();
			}
			hql="delete "+HistoryTask.class.getName()+" where taskId=:taskId";
			Query<?> query2 = session.createQuery(hql);
			query2.setParameter("taskId", task.getId());
