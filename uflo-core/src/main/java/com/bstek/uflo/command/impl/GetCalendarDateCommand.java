...
public class GetCalendarDateCommand implements Command<List<CalendarDate>> {
	...
	public List<CalendarDate> execute(Context context) {
		...
		CriteriaQuery<CalendarDate> query = cb.createQuery(CalendarDate.class);
		Root<CalendarDate> root = query.from(CalendarDate.class);
		query.select(root).where(cb.equal(root.get("calendarId"), calendarId));
