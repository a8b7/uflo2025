...
public class GetProcessByKeyCommand implements Command<ProcessDefinition> {
	...
	public ProcessDefinition execute(Context context) {
		...
		ProcessDefinition p=(ProcessDefinition)session.createQuery("from "+ProcessDefinition.class.getName()+" where key=:key", ProcessDefinition.class)
				.setParameter("key", processKey)
				.uniqueResult();
		if(p==null){
			return null;
		}else{
