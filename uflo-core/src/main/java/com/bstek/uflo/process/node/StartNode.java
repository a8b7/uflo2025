...
public class StartNode extends Node {
	...
	public void cancel(Context context, ProcessInstance processInstance) {
		...
		CriteriaBuilder cb = session.getCriteriaBuilder(); // 从 Session 获取 CriteriaBuilder
		CriteriaQuery<Task> query = cb.createQuery(Task.class); // 指定查询结果类型为 Task
		Root<Task> root = query.from(Task.class); // 设置查询的根实体（FROM Task）
// 添加条件：processInstanceId = ?（替代 Restrictions.eq）
		query.where(cb.equal(root.get("processInstanceId"), processInstance.getId()));
// 执行查询并获取结果（list() → getResultList()）
		Collection<Task> tasks = session.createQuery(query).getResultList();
