...
public class SubprocessNode extends Node {
	...
	public void cancel(Context context, ProcessInstance processInstance) {
		...
		CriteriaBuilder cb = context.getSession().getCriteriaBuilder(); // 从 Session 获取 CriteriaBuilder
		CriteriaQuery<ProcessInstance> query = cb.createQuery(ProcessInstance.class); // 指定查询结果类型为 Task
		Root<ProcessInstance> root = query.from(ProcessInstance.class); // 设置查询的根实体（FROM Task）
// 添加条件：processInstanceId = ?（替代 Restrictions.eq）
		query.where(cb.equal(root.get("parentId"), processInstance.getId()));
// 执行查询并获取结果（list() → getResultList()）
		Collection<ProcessInstance> subinstances = context.getSession().createQuery(query).getResultList();
